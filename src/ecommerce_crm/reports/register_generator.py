...
class GenerateRegister(View):
        ...
        try:
            for total in values:
                if total['bill__total_cost'] is not None:
                    self.bill_total = self.bill_total + \
                    total['bill__total_cost']
        ...
        try:
            try:
                if request.GET['all_registered_user']:
                    ...
                    (*self.fields_list).filter(is_superuser = 0)
                    return self.view_register(request)
            except:
                self.client_details = PurchaseOrder.objects.values\
                (*self.fields_list).filter(is_active = 1)
    ...
    def get(self,request):
        ...
        if request.GET.getlist('order'):
            ...
            self.selected_fields_client = self.selected_fields_client + \
        ...
        try:
            ...
            ('tax_name', flat = True).filter(id__in = request.GET.\
                getlist('surcharges'))
