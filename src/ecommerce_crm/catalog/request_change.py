...
from ecommerce_crm.voucher.models import VoucherId
from ecommerce_crm.voucher.models import FinancialSession
from ecommerce_crm.catalog.models import Bill
from ecommerce_crm.catalog.models import ChangeRequest
from ecommerce_crm.catalog.models import RequestSurchargeChange
from ecommerce_crm.catalog.models import RequestStatus
from ecommerce_crm.catalog.models import TaxesApplied
from ecommerce_crm.catalog.forms import ChangeRequestForm
...
from ecommerce_crm.config import _SENDER_EMAIL
from ecommerce_crm.config import _RECEIVER_EMAIL

from datetime import date, datetime

from ecommerce_crm.config import _ADMIN_GROUP
...
def request_save(request):
    ...
    if request.method == 'POST':
        ...
        get(purchase_order_id=purchase_order)
        ...
        for value in taxesapplied:
            if new_value_list[temp]:
                new_total=int(new_total) - int(previous_value_list[temp]) + \
                ...
                if RequestSurchargeChange.objects.filter(change_request=change_request):
                    ...
                    try:
                        ...
                        update(change_request=change_request,\
                            surcharge=surcharge,previous_value=previous_value_list[temp],\
...
def list_request(request):
    ...
    return render(request, 'catalog/list_request.html',\
        {'list':final_request_list})
...
def view_request(request):
    ...
    new_total = ChangeRequest.objects.values('new_total').\
    filter(id = request_id)[0]
    description = ChangeRequest.objects.values('description').\
    filter(id = request_id)[0]
    initiator = ChangeRequest.objects.values('initiator').\
    filter(id = request_id)[0]
    initiation_date = ChangeRequest.objects.values('initiation_date').\
    filter(id = request_id)[0]
    ...
    try:
        ...
        pass
    return render(request,'catalog/view_request.html',\
        {'previous_total':previous_total,'new_total':new_total,\
        'description':description,'id':request_id,\
